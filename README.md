...
- 集成中心：统一查看 SMTP、Lsky、Telegram、支付渠道、通知渠道、远程存储、Agent/Incus、OTA、扩展市场和主题市场状态，并保留一键健康检测历史。
- 计费账务：余额、在线充值、人工充值、支付回调、消费记录、余额调整、订单退款审批、插件支付网关原路退款工作台、对账、返利、积分、VIP 福利和礼品卡兑换。
- 资源风控：实例级评分、持续带宽、CPU 占用、包速率、扫描风险、QoS 降档、人工封禁、按来源实例限单/解除限单、分页运营列表和工单审核解除。
- 扩展中心：上传安装、在线市场安装、投稿审核、扫描、发布、启用、停用、卸载、配置表单、事件投递、Action runtime、扩展存储、开发模板和高风险 capability 审核。
...
主题包使用 `payincus.theme.json`，支持 CSS 入口、tokens、layoutSlots、templates、configSchema、预览、启用、回滚、受控配置表单、配置文件上传和主题市场投稿。后台主题中心入口为 `/admin/themes`，实际复用扩展中心主题管理页，便于管理员安装、预览、启用、配置和回滚主题。
