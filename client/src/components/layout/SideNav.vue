  ...
  >
    ...
    <nav class="nimbus-nav flex-1 overflow-y-auto">
      <template v-for="item in visibleMenuItems" :key="item.name || item.label">
        <!-- 分组标签 / 分隔线 -->
        <div v-if="item.divider" class="nimbus-group-row">
          <button
            v-if="!collapsed || mobileOpen"
            class="nimbus-group-label"
            type="button"
        ...
        <RouterLink
          v-else
          :to="item.path || '/'"
          class="nimbus-nav-item"
          :class="[
            collapsed && !mobileOpen ? 'is-icon-only' : '',
...
<style scoped>
/* ============================================================
   Nimbus sidebar — flat console surface, indigo signal
   ============================================================ */
...
.kawaii-sidebar .nimbus-brand-link.router-link-active,
.kawaii-sidebar .nimbus-brand-link.router-link-exact-active {
...
.nimbus-group-label:hover {
  color: var(--kawaii-muted);
  background: color-mix(in srgb, var(--kawaii-surface-soft) 80%, transparent);
}
...
.kawaii-sidebar .nimbus-nav-item.router-link-active:not(.is-active),
.kawaii-sidebar .nimbus-nav-item.router-link-exact-active:not(.is-active) {
  background: transparent !important;
...
.nimbus-nav-item.is-icon-only {
  justify-content: center;
  gap: 0;
  padding-left: 0;
  padding-right: 0;
}
.nimbus-nav-item.is-icon-only .nimbus-nav-indicator {
  left: 3px;
  height: 16px;
}
