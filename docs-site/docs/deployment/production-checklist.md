...
- `Public package ... cannot satisfy its minimum CPU/memory/disk requirement`：表示某个公开套餐仍在售，但绑定的在线宿主机扣除未删除实例占用后，已经无法满足该套餐最小可购买方案。闭环方式是增加宿主机可用 CPU、内存或磁盘，迁移/清理实例，给套餐绑定有余量的宿主机，或暂停该套餐公开销售；不要把这个告警当成前端显示问题处理。
